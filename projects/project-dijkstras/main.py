...
from network_routing import find_shortest_path_with_linear_pq, find_shortest_path_with_heap_pq
...
def main(seed: int, size: int, density: float, noise: float, distribution: str, source: int, target: int):
    ...
    start = time()
    path, cost = find_shortest_path_with_heap_pq(weights, source, target)
    end = time()
    heap_time = end - start
    print()
    print('-- Heap --')
